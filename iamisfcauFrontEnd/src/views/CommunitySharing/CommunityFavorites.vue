<template>
  <div class="fav">
    <div class="feed">
      <div v-for="post in favoritePosts" :key="post.id" class="post">
        <div class="post-head">
        ...
        <div class="actions">
          <button class="op" :class="{ active: post.userState.liked }" @click="onToggleLike(post)">
            <img
              class="opIcon"
          ...
          <button class="op" @click="openComment(post)">
            <img class="opIcon" src="/icons/comment.png" alt="comment" />
            <span>评论 {{ post.stats.comments }}</span>
          </button>

          <button class="op active" @click="removeFavorite(post)">
            <img class="opIcon" src="/icons/fav-active.png" alt="fav" />
            <span>已收藏</span>
          </button>
        </div>
      ...
      <div v-if="!loading && favoritePosts.length === 0" class="empty">
        你还没有收藏任何帖子～
      </div>
    ...
    <div v-if="commentDialog.visible" class="mask" @click.self="closeComment">
      <div class="dialog">
        <div class="dialogTitle">
          {{ commentDialog.replyTo ? '回复评论' : '发表评论' }}
        </div>
...
<script lang="ts" setup name="CommunityFavorites">
...
const favoritePosts = computed(() => posts.value.filter((p) => p.userState.favorited))
...
async function removeFavorite(post: CommunityPost) {
  try {
