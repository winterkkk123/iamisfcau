<template>
  <div class="page">
    ...
    <div class="queryBar">
      ...
      <button class="btn primary" @click="openApplyModal">新建请假</button>
    </div>
    ...
    <ul class="list">
      ...
      <li v-if="pagedRows.length === 0" class="empty">暂无符合条件的请假记录</li>
    </ul>
    ...
    <!-- 新建请假弹窗 -->
    <div v-if="applyVisible" class="modalMask" @click.self="closeApplyModal">
      <div class="modal">
        <div class="modalHeader">
          <div class="modalTitle">新建请假申请</div>
          <button class="closeBtn" @click="closeApplyModal">×</button>
        </div>

        <div class="modalBody">
          <div class="formRow">
            <span class="label">活动</span>
            <select class="select" v-model.number="applyForm.activityId">
              <option :value="0" disabled>请选择活动</option>
          ...
          <div class="formRow">
            ...
            <textarea class="textarea" v-model.trim="applyForm.reason" placeholder="请输入请假理由" />
          </div>
        </div>

        <div class="modalFooter">
          <button class="btn" @click="closeApplyModal">取消</button>
          <button class="btn primary" :disabled="!canSubmitApply" @click="submitApply">提交</button>
        </div>
      </div>
    </div>
  </div>
...
<script lang="ts" setup name="LeaveApply">
...
const sourceRows = computed(() => {
  if (me.value.role === '学生') return leaveApprovals.value.filter((x) => x.applicantId === me.value.id)
...
const canCancel = (r: LeaveApprovalItem) => r.status === '待审核' && r.applicantId === me.value.id

const cancelApply = async (applyId: string) => {
  try {
    await withdrawLeaveApproval(applyId, me.value.id)
    await loadData()
...
const applyForm = ref<{ activityId: number; reason: string }>({ activityId: 0, reason: '' })

const openApplyModal = () => {
  applyForm.value = { activityId: 0, reason: '' }
...
const canSubmitApply = computed(() => applyForm.value.activityId > 0 && !!applyForm.value.reason.trim())
...
.gridRow {
  display: grid;
  grid-template-columns: 1.1fr 0.8fr 1.6fr 1fr 0.7fr 180px;
  column-gap: 6px;
  align-items: center;
}
